...
int hook_connect(int sock, const struct sockaddr *addr, socklen_t len);
struct hostent * hook_gethostbyname(const char *name);
int hook_getaddrinfo(const char *node, const char *service,
                const struct addrinfo *hints,
                struct addrinfo **res);
void hook_freeaddrinfo(struct addrinfo *res);
struct hostent *hook_gethostbyaddr(const void *addr, socklen_t len,
                int type);
int hook_getnameinfo(const struct sockaddr *addr, socklen_t addrlen,
...
/*#define SETUP_SYM(X) \
    do { \
        rebind_symbols((struct rebinding[1]){{#X, X, (void *)&true_##X}}, 1); \
    } while(0)*/
...
static void get_chain_data(proxy_data * pd, unsigned int *proxy_count, chain_type * ct) {
    ...
    buff = strtok_r(strdup(config_str), "\n", &saveptr);
...
int hook_getaddrinfo(const char *node, const char *service, const struct addrinfo *hints, struct addrinfo **res) {
	int ret = 0;
...
int hook_getnameinfo(const struct sockaddr *sa,
...
#else
struct hostent *hook_gethostbyaddr(const char *addr, socklen_t len, int type) {
#endif
	static char buf[16];
	static char ipv4[4];
	static char *list[2];
